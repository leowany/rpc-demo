...
import com.leo.rpc.enums.RpcResponseCodeEnum;
...
public class RpcClientProxy implements InvocationHandler {
	...
	/**
	 * Used to send requests to the server.And there are two implementations: socket
	 * and netty
	 */
	...
	public Object invoke(Object proxy, Method method, Object[] args) {
		...
		RpcRequest rpcRequest = RpcRequest.builder().methodName(method.getName()).parameters(args)
				.interfaceName(method.getDeclaringClass().getName()).paramTypes(method.getParameterTypes())
				.requestId(UUID.randomUUID().toString()).build();
		RpcResponse<Object> rpcResponse = null;
		if (rpcRequestTransport instanceof NettyRpcClient) {
			CompletableFuture<RpcResponse<Object>> completableFuture = (CompletableFuture<RpcResponse<Object>>) rpcRequestTransport
